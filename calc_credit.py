...
month = ['styczniu', 'lutym', 'marcu', 'kwietniu', 'maju', 'czerwcu', 'lipcu', 'sierpniu', 'wrześniu', 'październiku', 'listopadzie', 'grudniu']*2
...
if (isinstance(a, float) and isinstance(b, float) and isinstance(c, float)) == True and (a and b and c) != 0:
	for i in range(24):
		...
		calc_less = a-calc
		a = calc
		print('W '+month[i]+' twoja pozostała kwota kredytu to '+str(calc)+', to '+str(calc_less)+' mniej niz w poprzednim miesiącu\n')
else:
